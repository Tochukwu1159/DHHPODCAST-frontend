...
import { useLoginUser, useResetPasswordWithoutToken } from "../api/user";
  ...
  const SignIn = ({ setSignInOpen, setSignUpOpen }) => {
    ...
    const {mutate: login, isPending:lendingLonging} =  useLoginUser()
    const {mutate: resetPassword, isPending:resetPending} =  useResetPasswordWithoutToken()
    ...
    const performResetPassword = () => {
      ...
      resetPassword({email, newPassword:newpassword, confirmPassword:confirmedpassword})
      // if (otpVerified) {
      //   setShowOTP(false);
